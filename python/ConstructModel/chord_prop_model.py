...
initial_twist = np.interp(span_loc, [9.07347e+01, 9.31224e+01],
                           [-1.62330e+00, -1.88437e+00 ], left=np.nan, right=np.nan)
...
###########################################
##### Output the chord length and pitch axis interpolated to the desired span location.

# Pitch axis values from here:
# https://github.com/IEAWindTask37/IEA-15-240-RWT/blob/master/WT_Ontology/IEA-15-240-RWT.yaml
...
print('Interpolated pitch axis value: (negative of this goes in the translation displacement of the .yaml)')
print(pitch_axis)
...
print('Interpolated chord value (use for test velocities and scaling)')
print(chord_length)
...
print('Assumed: span_loc={}, span_frac={}'.format(span_loc, span_frac))
print('Verify that these match the next output based on the node number. \n')
...
with open(out_3dof, 'w') as f:
    ...
    f.write('# - name: move_to_pitch_axis \n')
